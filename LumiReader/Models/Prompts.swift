...
"""),
        // 列出案例
        Prompt(title: "列出案例", content: """
        请你用大白话，给我讲讲文中主要有哪些有意思的故事或案例？这些故事或案例给我们带来了什么启发，或者教会了我们什么道理？ 语言要简单易懂，就像给朋友讲事情一样。
        """),
        // 列出数据
        Prompt(title: "列出数据", content: """
请你充当数据提取专家，帮我从下面这篇文章中找出所有重要的数字、百分比、比例、金额、时间、数量等关键统计信息。
请以项目符号或编号列表的形式，清晰地展示这些数据。如果数据之间存在关联，可以考虑用表格形式展示，以便更好地理解。
对于每个数据点：
请列出具体的数值。
简要说明这个数据代表什么（例如：某项增长率、某群体的比例、某个年份的销售额）。
指出它关联的上下文（例如：在哪个研究中发现的、哪一年份的数据）。
"""),
        // 列出金句
...
请你扮演一位资深的时事评论员，用你最犀利但又不失理性的眼光，对下面这篇文章进行一番深度剖析。请你像用手术刀一样精准地剥开文章的表层，不带任何感情色彩和个人偏好，只基于事实和逻辑来评论。
你的评论将分为两部分，但都要确保语言是最接地气的大白话，像在跟街坊邻居聊天一样，让没有专业背景的人也能听懂、听明白你的高见。
一、对文章本身的评论：
直指核心： 文章的核心论点和主要观点是什么？它说得有没有道理？有没有经得起推敲的地方？
独到见解： 它有没有遗漏什么关键的视角？或者有哪些深层的问题它没有点明，但你觉得很重要？
优点与不足： 文章哪些地方分析得特别透彻、有启发性？又有哪些地方可能存在逻辑漏洞、论据不足，或者过于片面？
实际意义： 这篇文章对普通大众有什么实际的意义或影响？我们读完之后应该如何看待？
二、对文中主要人物/观点的评论：
如果文章中涉及到了主要人物、提出核心观点的个体或组织，请你也用同样犀利而理性的方式，对他们的观点或行为进行一番点评。
他们的观点/行为是否站得住脚？其逻辑是什么？有没有自相矛盾的地方？
这些观点/行为是一时兴起还是有深层动机或背后考量？
这些观点/行为对文章的主题或整体事件产生了什么影响？是推动还是阻碍？
从他们的观点/行为中，我们又能得到什么启发或警示？
"""),
        // 科普[x]
